                       ...
                       gboolean *error)
{
    ...
    while ((dirent = readdir(dir)) != NULL) {
        dname = g_utf8_normalize (dirent->d_name, -1, G_NORMALIZE_NFC);
        ...
        if (rc == 0) {
            if (S_ISREG(dent1->st.st_mode) && !dent2->is_dir) {
                if (dent1->st.st_mtime != dent2->ce->ce_mtime.sec)
                    *mod = g_list_prepend (*mod, g_strconcat(dir_path, "/", dent1->dname, NULL));
            } else if ((S_ISREG(dent1->st.st_mode) && dent2->is_dir) ||
                       (S_ISDIR(dent1->st.st_mode) && !dent2->is_dir)) {
                *add = g_list_prepend (*add, g_strconcat(dir_path, "/", dent1->dname, NULL));
                *del = g_list_prepend (*del, g_strconcat(dir_path, "/", dent1->dname, NULL));
            }
            p1 = p1->next;
            p2 = p2->next;
        } else if (rc < 0) {
            ...
            *del = g_list_prepend (*del, g_strconcat(dir_path, "/", dent2->dname, NULL));
            p2 = p2->next;
        }
    }
